...
2) 工厂模式：
   再创建一个工具类，名为Factory。将Factory作为中间层，如果dao对象名(类名)发生变化，只需要更改Factory层中的名字即可，service层不用变化，可以大大减小耦合度。
3) 反射机制：javaSE教程
### 简单手写底层原理
...
区别：  
实际web开发中，主要用的是ApplicationContext而不是BeanFactory，这是因为BeanFactory在读取配置文件的时候不会创建对象，在getBean获取对象的时候才会创建对象，虽然能够使项目启动得较快，但是在使用过程中由于需要创建对象导致运行效率变慢。
...
#### 内部属性注入bean对象
内部属性注入通常是匿名、只能被包含它的外部 bean 使用，不能复用；外部属性注入（通过 ref 引用）是独立定义的 bean，可在多个地方复用
#### 级联属性注入bean对象
#### 注入集合类型属性
...
#### bean的作用域
作用域分为单例 和 多例
1) 单例：getBean()只会返回同一个bean对象
2) 多例：getBean()会返回新的bean对象。

一般情况下Spring默认为单例，若要配置为多例，须在bean标签中写上scope=prototype。
### Spring生命周期(1~3年经验)
大概流程：了解即可
1) 第一步：实例化bean对象(底层通过反射机制)
2) 第二步：属性赋值(底层通过反射机制)   
(后置处理器中执行在init前的方法)
3) 第三步：调用初始化方法init  
(后置处理器中执行在init后的方法)
4) 第四步：正常使用对象
5) 第五步：关闭Ioc容器，调用destroy方法销毁对象。
#### bean的后置处理器
定义一个类，实现BeanPostProcessor(后置处理器)接口，该接口中有两个方法，
这两个方法会在第三步：调用初始化方法init 前后依次执行。也就是说，在上面的五步生命周期里，在init前后加入了两个后置处理器执行的方法。
### bean的自动装配
自动装配能够自动注入bean对象的属性，装配方式有两种：
1) byName：根据属性的id进行装配，要求id必须与bean的属性名相同。
2) byType：根据属性的类型进行装配，要求同一类型的属性不能有多个，否则不知道装配哪一个。